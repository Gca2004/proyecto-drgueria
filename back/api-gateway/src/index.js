...
// MS Notificaciones — Puerto 3005
app.use('/v1/notificaciones', createProxyMiddleware({
  target: process.env.MS_NOTIFICACIONES_URL,
  changeOrigin: true,
  pathRewrite: (path) => '/notificaciones' + path,
  on: {
    error: (err, req, res) => {
      console.error('❌ Error proxy MS Notificaciones:', err.message);
      res.status(502).json({ error: 'MS Notificaciones no disponible', detalle: err.message });
    }
  }
}));
...
app.get('/health', (req, res) => {
  res.status(200).json({
    ...
    microservicios: {
      ...
      compras:        process.env.MS_COMPRAS_URL        || 'no configurado',
      pagos:          process.env.MS_PAGOS_URL          || 'no configurado',
      notificaciones: process.env.MS_NOTIFICACIONES_URL || 'no configurado',
...
app.use((req, res) => {
  res.status(404).json({
    ...
    rutas_validas: ['/v1/usuarios', '/v1/productos', '/v1/proveedores', '/v1/pedidos', '/v1/compras', '/v1/pagos', '/v1/notificaciones', '/v1/auditoria', '/v1/reportes']
  });
});

const PORT = process.env.PORT || 4000;
app.listen(PORT, () => {
  console.log(`🚀 API Gateway corriendo en http://192.168.100.1:${PORT}`);
  ...
  console.log(`   /v1/compras        → ${process.env.MS_COMPRAS_URL}`);
  console.log(`   /v1/pagos          → ${process.env.MS_PAGOS_URL}`);
  console.log(`   /v1/notificaciones → ${process.env.MS_NOTIFICACIONES_URL}`);
